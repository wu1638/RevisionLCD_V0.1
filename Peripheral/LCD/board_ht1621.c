...
void HT1621_WriteData4Bit2(uint8_t addr, uint8_t data);
void HT1621_WriteData8Bit2(uint8_t addr, uint8_t data);
...
void HT1621_Init2(void)
{
   ...
   HT1621_WriteCommand2(HT1621_SYS_EN); // 打开系统振荡器
   HT1621_WriteCommand2(HT1621_BIAS);   // BIAS 13 4个公共口
   HT1621_WriteCommand2(HT1621_RC256);  // 使用RC_256K系统时钟源，片内RC振荡器
   HT1621_WriteCommand2(HT1621_WDT_DIS);
   HT1621_WriteCommand2(HT1621_LCD_ON);
}
/**
 @brief 写入命令，命令标志100
 @param cmd -[in] 命令数据
...
static void gpioConfig(void)
{
    ...
    gpio_init_struct.Pin = HT1621_CS2_PIN;
    gpio_init_struct.Mode = GPIO_MODE_OUTPUT_PP;
    gpio_init_struct.Speed = GPIO_SPEED_FREQ_HIGH;
    HAL_GPIO_Init(HT1621_CS2_PORT, &gpio_init_struct);
